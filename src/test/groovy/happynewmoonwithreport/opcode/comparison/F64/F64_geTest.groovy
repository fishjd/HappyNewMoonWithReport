 ...
 */
class F64_geTest extends Specification {
	void setup() {
	}
	...
	def "Execute F64 less than to #val1 | #val2 || #expected "(Float val1, Float val2, Integer expected) {
		...
		F64_ge opcode = new F64_ge(instance);
 ...
 */
	...
	def "Execute F64_ge throw exception on incorrect Type on second param "() {
		...
		F64_ge function = new F64_ge(instance);
		...
		exception.getUuid().toString().contains("76d88e00-c903-487e-aed8-7035e6e5a062");
	}

	def "Execute F64_ge throw exception on incorrect Type on first param "() {
