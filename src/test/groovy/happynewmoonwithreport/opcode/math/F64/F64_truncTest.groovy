 ...
 */
class F64_truncTest extends Specification {
	String inputType;
	String returnType;
 ...
 * @return None.
 */
	def "Execute F64_trunc with ##count -> #val1 || #expected "(Integer count, Double val1, Double expected) {
		setup: " push two values on stack."

		WasmInstanceInterface instance = new WasmInstanceStub();
		instance.stack().push(new F64(val1));

		F64_trunc opcode = new F64_trunc(instance);
	 ...
	 * @return None.
	 */
	def "Execute F64 trunc #count | #val1_s  || #expected "(Integer count, String val1_s, String expected) {
	...
	def "Execute F64_trunc Canonical"() {
		setup: " push ONE value on stack."
	...
	def "Execute F64_trunc throws exception on incorrect Type on first param "() {
		setup: " a value of F64  value"
		WasmInstanceInterface instance = new WasmInstanceStub();
		instance.stack().push(new I64(3));  // value 1

		F64_trunc function = new F64_trunc(instance);
		...
		exception.getUuid().toString().contains("5fed2dfd-37cc-409d-a1b8-1b424b3563fa");
	}
